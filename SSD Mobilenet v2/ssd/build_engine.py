...
Input : ssd_mobilenet_v2_cars.pb
Output: TRT_ssd_mobilenet_v2_cars.bin
"""
...
MODEL_SPECS = {
    'ssd_mobilenet_v2_cars': {
        'input_pb':   os.path.abspath(os.path.join(
                          DIR_NAME, 'ssd_mobilenet_v2_cars.pb')),
        'tmp_uff':    os.path.abspath(os.path.join(
                          DIR_NAME, 'tmp_v2_cars.uff')),
        'output_bin': os.path.abspath(os.path.join(
                          DIR_NAME, 'TRT_ssd_mobilenet_v2_cars.bin')),
        'num_classes': 2,
        'confthresh': 1e-8,
        'min_size': 0.05,
        'max_size': 0.95,
        'aspectRatios' : [1.0, 2.0, 0.5, 3.0, 0.4],
...
def add_plugin(graph, model, spec):
    """add_plugin
    Reference:
    https://devtalk.nvidia.com/default/topic/1050465/jetson-nano/how-to-write-config-py-for-converting-ssd-mobilenetv2-to-uff-format/post/5333033/#5333033
    """
